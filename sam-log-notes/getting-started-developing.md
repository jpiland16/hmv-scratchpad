...
5. Configure the transformation quaternions to align the limbs correctly.
    - You can use the 'tinker' tool (At the website homepage domain + '/tinker/) to upload a quaternion data file and mess with the limbs in order to get them in a reasonable position where the movements make sense. A bone will only be moved by quaternion data if it has at least one quaternion assigned to it--otherwise, it will follow its parent's orientation. Column numbers are going to be separated by multiples of 4 since the file has columns of quaternions.
    - I recommend doing this one limb at a time, starting with ROOT and proceeding in topological order. 
