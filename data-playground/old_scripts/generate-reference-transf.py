"""Attempts to generate data such that I know what it will look like
when we graph it. It makes the x-axis trace out a circle in the xy-plane.
"""
...
def generate_data_new():
	...
	while (t_curr < t_final):
		...
		print("orientation matrix to point in direction {0}:\n{1}".format(arm_direction, rotation_mat))
...
def create_printed_data(output_filepath):
	...
	# data_quats = generate_simple_circle()
...
def generate_simple_circle():
	generated_quats = []
	for i in range(30):
		t_curr = i*(1/30.)
		theta_curr = 2*np.pi*t_curr
		generated_quats.append(np.array([np.cos(theta_curr/2), np.sin(theta_curr/2), 0, 0]))
	return generated_quats
...
if __name__=='__main__':
	...
	create_printed_data(sys.argv[1])